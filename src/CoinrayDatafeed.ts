...
export class CoinrayDatafeed implements Datafeed
{
    ...
    async getHistoryKLineData(symbol: SymbolInfo, period: Period, from: number, to: number): Promise<KLineData[]> {
        ...
        to /= 1000;
        if (!this.firstLoad) {
            to = to + (period.multiplier * this.timespanToSeconds(period.timespan));
        }
        console.info({ symbol, period, from, to })
        const candles = await this.api.fetchCandles({ coinraySymbol: `${symbol.exchange}_${symbol.ticker ?? symbol.shortName}`, resolution: `${period.multiplier * this.timespanToSeconds(period.timespan)}`, start: from, end: to, useWebSocket: false });
        console.info(candles)

        if (!this.firstLoad && candles.length > 0) {
            this.lastCandle = candles.pop() || null;
        }
    ...
    subscribe(symbol: SymbolInfo, period: Period, callback: DatafeedSubscribeCallback): void {
        this.api.subscribeCandles({ coinraySymbol: `${symbol.exchange}_${symbol.ticker ?? symbol.shortName}`, resolution: `${period.multiplier * this.timespanToSeconds(period.timespan)}`, lastCandle: this.lastCandle || undefined }, (data: SubscribeCandlesData) => {
            const candle = {
                ...data.candle,
    ...
    unsubscribe(symbol: SymbolInfo, period: Period): void {
        this.api.unsubscribeCandles({ coinraySymbol: `${symbol.exchange}_${symbol.ticker ?? symbol.shortName}`, resolution: `${period.multiplier * this.timespanToSeconds(period.timespan)}` });
    }

    private timespanToSeconds(timespan: string): number {
        switch (timespan) {
